...
import { defaultDishList, dishToStationMap, spacecraftMap } from '../data/referenceData';
...
export function getStationByDishName(dishName: string): 'Goldstone' | 'Canberra' | 'Madrid' {
...
function serializeResponseDish(dish: ResponseDish): Dish {
	...
	const targets = (Array.isArray(target) ? target : [target]).filter((x) => !!x);
...
export const determineTargetStatus = (dish: Dish): 'NONE' | 'IDLE' | 'ACTIVE' => {
	if (!dish) {
		return 'NONE';
	}
...
export const getSummarizedDataByStation = (
	...
	summarizedDishList = dishes.filter(
		(dish) => dish.metadata.station.toLowerCase() === stationName && dish['targets'].length > 0
	);

	// Only include signal status
	summarizedDishList = summarizedDishList.map((dish) => {
		const abbreviatedTargets = dish['targets'].map((target: TargetEntry) => target['@name']);
...
export const diffDataByDish = (
	prev: DSNDataInterface,
	next: DSNDataInterface
): TargetDiffByDish => {
	let diff = {};
	if (!!prev && !!next) {
		for (let dish of next.dishes) {
			const prevDish = prev.dishes.find((d) => d['@name'] === dish['@name']);
			const prevTargets = prevDish.targets.map((t) => t['@id']);
			const nextTargets = dish.targets.map((t) => t['@id']);
			const newTargets = nextTargets.filter((targetID) => !prevTargets.includes(targetID));
			diff[dish['@name']] = newTargets;
		}
	}
	return diff;
};
