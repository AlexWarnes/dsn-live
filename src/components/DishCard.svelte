...
<span class="rel dish-card-wrapper">
	...
	>
		...
		<div class="dish-card-footer">
			...
			{#if !!dish['@updated']}
				<span class="subtitle"
					>Updated: {dish['@updated'] ? new Date(dish['@updated']).toLocaleString() : ''}</span
				>
			{/if}
		</div>
	</article>
    ...
    /> -->
			<OverflowMenu flipped>
				...
				<OverflowMenuItem on:click={() => (showReferencesModal = true)}>Learn More</OverflowMenuItem
				>
			</OverflowMenu>
		</span>
	{/if}
</span>
<ReferenceListModal open={showReferencesModal} {targetDetails} closeFn={closeReferencesModal} />
...
<style>
	...
	.abs.spinner {
		left: 7px;
		bottom: 2px;
		/* Font size fixes wobbly spinner */
		font-size: 14px;
	}
	.abs.buttons {
	...
	.dish-card-container :global(.bx--content-switcher-btn:first-child) {
		border-radius: 0;
	}
	...
	.dish-card-container
		:global(.bx--content-switcher--selected + .bx--content-switcher-btn::before) {
